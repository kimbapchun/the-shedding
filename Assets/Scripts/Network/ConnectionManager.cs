...
namespace TheShedding.Network
{
    ...
    public class ConnectionManager : MonoBehaviour
    {
        ...
        private void HandleStopped()
        {
            ...
            // DisconnectReason은 두 가지가 섞여 나온다(NetworkConnectionManager.GetDisconnectReason).
            //  ① 서버가 의도적으로 보낸 사유 — DisconnectClient(id, reason)이나 연결 승인 거부 시.
            //     우리가 서버일 때 직접 채우는 값이므로 그대로 사용자에게 보여줘도 된다.
            //  ② 그 외 상황에서 NGO가 채우는 영문 진단 문자열.
            //     "[Disconnect Event][Client-1][TransportClientId-0]..." 형태라 사용자에게 부적합하다.
            // 둘을 구분하는 공개 API가 없어서 진단 문자열의 고정 접두사로 판별한다.
            ...
            {
                // 사용자에게는 감추되 원인 추적은 가능하도록 로그로만 남긴다.
